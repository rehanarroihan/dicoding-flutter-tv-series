...
void main() {
  const API_KEY = '2174d146bb9c0eab47529b2e77d6b526';
...
''';
  ...
  group('get Popular TV Series', () {
    ...
    test('should return list of TV Series Model when the response code is 200',
      ...
      // arrange
      when(mockHttpClient
              .get(Uri.parse('$BASE_URL/tv/popular?api_key=$API_KEY')))
  ...
  group('get Top Rated TV Series', () {
    ...
    test(
      ...
      // arrange
      when(mockHttpClient
              .get(Uri.parse('$BASE_URL/tv/top_rated?api_key=$API_KEY')))
          .thenAnswer(
  ...
  group('get Now Showing Today TV Series', () {
    final tTvSeriesList = TvSeriesResponse.fromJson(
      json.decode(readJson('dummy_data/tv_series_now_showing_today.json')),
    ).tvSeriesList;

    test(
        'should return list of Now Showing Today TV Series when response code is 200 ',
        () async {
      // arrange
      when(mockHttpClient
          ...
          .thenAnswer(
        (_) async => http.Response(
          readJson('dummy_data/tv_series_now_showing_today.json'),
          200,
        ),
      );
      // act
      final result = await dataSource.getShowingTvSeries();
      // assert
      expect(result, tTvSeriesList);
    });
    ...
    test('should throw ServerException when response code is other than 200',
      ...
      when(mockHttpClient
          ...
          .thenAnswer((_) async => http.Response('Not Found', 404));
      // act
      final call = dataSource.getShowingTvSeries();
      // assert
      expect(() => call, throwsA(isA<ServerException>()));
    });
  });
  ...
  group('search TV Series', () {
    ...
    test('should search TV Series when response code is 200', () async {
      ...
      when(
        mockHttpClient.get(
            Uri.parse('$BASE_URL/search/tv?api_key=$API_KEY&query=$tQuery')),
      ).thenAnswer((_) async => http.Response(jsonString, 200));
      // act
